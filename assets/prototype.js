...
(function () {
  ...
  const Views = {
    register() {
      ...
      if (cur.k==='business') form = `
        ...
        <div class="table-wrap"><table class="tbl"><thead><tr><th>No</th><th>Branch</th><th>Department</th><th>Location</th><th>Email</th><th>Phone Number</th><th style="text-align:right">Action</th></tr></thead><tbody>
          <tr><td>1</td><td>Headquarter</td><td style="font-size:12.5px;color:var(--muted)">Maintenance, IT, HRD, Marketing, Finance, Legal, Operation</td><td>Jakarta, Indonesia</td><td>info@seruam.com</td><td>081234567890</td><td style="text-align:right"><div class="tbl-actions" style="justify-content:flex-end"><button title="Edit" data-toast="Edit — coming soon in this prototype.">${eIc}</button><button title="Delete" data-toast="Delete — coming soon in this prototype.">${tIc}</button></div></td></tr>
        </tbody></table></div>`;
      }
      else {
        ...
        <div class="table-wrap"><table class="tbl"><thead><tr><th>No</th><th>Name</th><th>Email</th><th>Branch</th><th>Departement</th><th>Job Title</th><th>Role</th><th style="text-align:right">Action</th></tr></thead><tbody>
          ${tm.map((m,i)=>`<tr><td>${i+1}</td><td><div class="cell-user"><img class="avatar" src="assets/p${(i%6)+1}.jpg" alt=""><div class="nm">${m[0]}${i===0?' (you)':''}</div></div></td><td>${m[1]}</td><td>${m[2]}</td><td>${m[3]}</td><td>${m[4]}</td><td><span class="badge ${rb[m[5]]}">${m[6]}</span></td><td style="text-align:right"><div class="tbl-actions" style="justify-content:flex-end"><button title="Edit" data-toast="Edit — coming soon in this prototype.">${eIc}</button>${i===0?`<button title="Manage seat" data-toast="Manage seat — coming soon in this prototype.">${mvIc2}</button>`:`<button title="Delete" data-toast="Delete — coming soon in this prototype.">${tIc}</button>`}</div></td></tr>`).join('')}
        </tbody></table></div>
    ...
    billing() {
      ...
      <div class="grid" style="grid-template-columns:1.35fr 1fr;margin-top:16px;align-items:start">
        <div class="card card-pad">
          <div class="between"><div><div class="section-title" style="font-size:16px">Current Subscription</div><p class="muted" style="font-size:13px;margin-top:2px">${active?'You are currently on a Paid Plan':'You are currently in Trial Plan'}</p></div>
            <div style="display:flex;gap:10px;flex-wrap:wrap"><button class="btn btn-outline btn-sm" id="changeCycle">Change Cycle</button>${active?`<button class="btn btn-outline btn-sm" id="addSeats">Add Seats</button><button class="btn btn-outline btn-sm" id="removeSeats">Remove Seats</button>`:`<button class="btn btn-outline btn-sm" data-go="plan">Upgrade <span class="mobile-only"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg></span></button>`}</div></div>
        ...
        <div class="card card-pad">
          ...
          <button class="btn btn-outline btn-block" style="margin-top:18px" data-toast="Update payment method — coming soon in this prototype.">Update Payment Method</button>
        </div>
      </div>
      <div class="card card-pad" style="margin-top:18px">
        <div class="between"><div><div class="section-title" style="font-size:16px">Invoice</div><p class="muted" style="font-size:13px;margin-top:2px">View your previous invoices/</p></div><button class="btn btn-outline btn-sm" data-toast="Downloading all invoices...">${dl} Download All</button></div>
  ...
  function openModal(kind, ctx) {
    ...
    if (kind === 'addSeats' || kind === 'removeSeats') {
      ...
      const card = (key,label,priceLine,note,badge) => `<div data-cycle="${key}" style="cursor:pointer;display:flex;align-items:center;gap:12px;border:${sel===key?'1.5px solid var(--blue)':'1px solid var(--line)'};background:${sel===key?'var(--blue-soft)':'#fff'};border-radius:12px;padding:14px 16px;margin-top:12px">
        ...
        <span style="width:20px;height:20px;border-radius:50%;flex:none;border:${sel===key?'none':'1.5px solid var(--line)'};background:${sel===key?'var(--blue)':'#fff'};display:flex;align-items:center;justify-content:center">${sel===key?'<span style="width:8px;height:8px;border-radius:50%;background:var(--surface)"></span>':''}</span></div>`;
  ...
  document.addEventListener('click', (e) => {
    ...
    const pgb = e.target.closest('.pager button');
    if (pgb) { const t = pgb.textContent.trim(); if (/^\d+$/.test(t)) { pgb.parentElement.querySelectorAll('button').forEach(x=>x.classList.remove('active')); pgb.classList.add('active'); } else { toast('Pagination is illustrative in this prototype.'); } return; }
